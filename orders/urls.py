...
urlpatterns = [
    ...
    path(
        "admin/bulk-update/",
        AdminBulkOrderUpdateView.as_view(),
        name="admin-bulk-update",
    ),
    path(
        "admin/<int:order_id>/update-status/",
        AdminOrderStatusUpdateView.as_view(),
        name="admin-update-status",
    ),
    path(
        "checkout/<str:provider>/",
        CreateCheckoutView.as_view(),
        name="payment-checkout",
    ),
    path(
        "webhook/<str:provider>/", PaymentWebhookView.as_view(), name="payment-webhook"
    ),
    path(
        "payments/history/",
        PaymentTransactionHistoryView.as_view(),
        name="payment-history",
    ),
    path(
        "payments/<str:transaction_id>/verify/",
        VerifyPaymentView.as_view(),
        name="payment-verify",
    ),
    path(
        "<str:order_number>/refund/", RefundPaymentView.as_view(), name="order-refund"
    ),
    path(
        "<int:order_id>/status/",
    ...
    path(
        ...
        views.OrderStatusUpdateView.as_view(),
        name="order-status-update-by-number",
    ),
]
