...
class StripeProvider(PaymentProvider):
    def __init__(self, api_key: str, webhook_secret: str = None):
        if api_key and not api_key.startswith("sk_test_"):
            raise ValueError("StripeProvider only accepts Stripe TEST secret keys (they start with 'sk_test_').")
        self.api_key = api_key
        self.webhook_secret = webhook_secret
        stripe.api_key = api_key

    def initialize_payment(self, amount: Decimal, currency: str = "usd", **kwargs) -> PaymentResult:
        ...
        return PaymentResult(
            success=True, transaction_id=session.id, amount=amount, currency=currency,
    ...
    def verify_payment(self, transaction_id: str) -> PaymentResult:
        ...
        return PaymentResult(
            success=paid, transaction_id=session.id, message=session.payment_status,
    ...
    def refund_payment(self, transaction_id: str, amount: Optional[Decimal] = None) -> PaymentResult:
        try:
            ...
            return PaymentResult(success=False, message=str(e))
        return PaymentResult(success=refund.status in ("succeeded", "pending"), transaction_id=refund.id, message=refund.status)
