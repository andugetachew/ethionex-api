...
def tracked_order(db, buyer):
    return Order.objects.create(
        ...
        status="shipped", subtotal=1000, total=1000,
...
def pending_order(db, buyer):
    return Order.objects.create(
        user=buyer, full_name="Buyer", phone_number="09",
        address="Addr", city="City", payment_method="cash",
        status="pending", subtotal=500, total=500,
    )
...
class TestTrackOrderView:
    ...
    def test_track_by_order_number(self, auth_client, tracked_order):
        response = auth_client.get(
            "/api/v1/orders/track/",
            {"order_number": tracked_order.order_number}
        )
...
class TestAdminBulkOrderUpdateView:
    ...
    def _make_orders(self, buyer, n=2):
        return [
            Order.objects.create(
                user=buyer, full_name="B", phone_number="09",
                address="A", city="C", payment_method="cash",
    ...
    def test_missing_status_returns_400(self, client, admin, buyer):
        ...
        client.force_authenticate(user=admin)
        response = client.post(
            self.URL, {"order_ids": [orders[0].id]}, format="json"
        )
        assert response.status_code == 400
